...
//    }
    ...
    fun copy(copy: Supplier<out BoulderConfig>): Builder {
      return copy(copy.get())
    }

    fun copy(copy: BoulderConfig): Builder {
...
//      }
    ...
    val CODEC = RecordCodecBuilder.create { codecRecorder: RecordCodecBuilder.Instance<BoulderConfig> ->
      codecRecorder.group(
        BlockStateProvider.CODEC.fieldOf("block_provider")
          .forGetter { config: BoulderConfig -> config.blockProvider },
        BlockStateProvider.CODEC.fieldOf("middle_block_provider")
          .forGetter { config: BoulderConfig -> config.middleBlockProvider },
        BlockStateProvider.CODEC.fieldOf("top_block_provider")
          .forGetter { config: BoulderConfig -> config.topBlockProvider },
        IntProvider.CODEC.fieldOf("stackHeight")
          .forGetter { config: BoulderConfig -> config.stackHeight },
        RadiusSettings.CODEC.fieldOf("radius_settings")
          .forGetter { config: BoulderConfig -> config.radiusSettings },
        Codec.DOUBLE.fieldOf("radius_divisor_per_stack").orElse(1.0)
          .forGetter { config: BoulderConfig -> config.radiusDivisorPerStack },
        Codec.FLOAT.fieldOf("noise_frequency").orElse(1.0f)
          .forGetter { config: BoulderConfig -> config.noiseFrequency },
        Codec.list(BlockPredicate.CODEC).fieldOf("predicate")
          .orElse(ArrayList())
          .forGetter { config: BoulderConfig -> config.predicate },
...
//          .orElse(Fluids.EMPTY.defaultFluidState()).forGetter { config: NoisySphereConfig -> config.fluidState },
        Codec.DOUBLE.fieldOf("2d_noise_chance").orElse(0.25)
          .forGetter { config: BoulderConfig -> config.noise2DChance },
      ...
      ).apply(codecRecorder, ::BoulderConfig)
    }
  }
}