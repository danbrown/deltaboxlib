...
object ShipAssembler {
  fun createShip(level: ServerLevel, center: BlockPos, predicate: (BlockState) -> Boolean, scale: Double = 1.0): ServerShip? {
    val blocks = DenseBlockPosSet()
    blocks.add(center.x, center.y, center.z)
    val result = canCreateShip(level, center, blocks, predicate)
    if (result) {
      val createdShip: ServerShip = VSUtils.ShipAssembly.createNewShipWithBlocks(center, blocks, level, scale)
      // assign the shipControl
      val shipControl = SpaceShipControl.getOrCreate(createdShip)
      // update the shipControl information with the blocks present
      shipControl.updateShipControl(level, blocks)
      return createdShip
    }
    else {
      return null
  ...
  fun unfillShip(level: ServerLevel, ship: ServerShip, direction: Direction, shipCenter: BlockPos, center: BlockPos) {
    ...
    level.players()
      .forEach { player -> PacketStopChunkUpdates(chunkPosesJOML).sendToClient(VSUtils.getPlayerWrapper(player)) }
        ...
        for (x in 0..15) {
          for (y in 0..15) {
            for (z in 0..15) {
              ...
              val inWorldPos = shipToWorld.transformPosition(alloc0.set(realX + 0.5, realY + 0.5, realZ + 0.5))
                .floor()
              ...
              VSUtils.RelocationUtil.relocateBlock(level, inShipPos, inWorldBlockPos, false, null, rotation, direction)
            }
          }
        }
      }
    ...
    for (triple in toUpdate) {
      VSUtils.RelocationUtil.updateBlock(level, triple.first, triple.second, triple.third)
    }

    VSUtils.executeIf(level.server,
      ...
      { chunkPoses.all { chunkPos -> VSUtils.isTickingChunk(level, chunkPos) } }
    ) {
      // Once all the chunk updates are sent to players, we can tell them to restart chunk updates
      level.players()
        .forEach { player ->
          PacketRestartChunkUpdates(chunkPosesJOML).sendToClient(VSUtils.getPlayerWrapper(player))
        }
    }
  }

  private fun canCreateShip(
    level: ServerLevel,
    start: BlockPos,
    ...
    if (ProjectConfig.SERVER.maxShipBlocks <= 0) return false

    directions(start) { stack.push(it) }

    while (!stack.isEmpty && blocks.size < ProjectConfig.SERVER.maxShipBlocks) {
      val pos = stack.pop()
    ...
    if ((blocks.size >= ProjectConfig.SERVER.maxShipBlocks)) {
      ProjectContent.LOGGER.info("Can't assemble ship, ${blocks.size} blocks found, out of ${ProjectConfig.SERVER.maxShipBlocks} allowed")
      return false
    }

    ProjectContent.LOGGER.info("Assembled ship with ${blocks.size} blocks, out of ${ProjectConfig.SERVER.maxShipBlocks} allowed")
    return true
  }

  private fun directions(center: BlockPos, lambda: (BlockPos) -> Unit) {
    if (!ProjectConfig.SERVER.diagonals) Direction.values()
      .forEach { lambda(center.relative(it)) }
    for (x in -1..1) {
      for (y in -1..1) {
        for (z in -1..1) {
          if (x != 0 || y != 0 || z != 0) {
