...
fun AABBic.toMinecraft() = AABB(minX().toDouble(), minY().toDouble(), minZ().toDouble(), maxX().toDouble(), maxY().toDouble(), maxZ().toDouble())
...
fun PoseStack.multiply(modelTransform: Matrix4dc, normalTransform: Quaterniondc) = also {
  val last = last()
  val newPose = Matrix4d().set(last.pose())
    .mul(modelTransform)
  val newNormal = last.normal()
    .mul(Matrix3f().set(normalTransform))
