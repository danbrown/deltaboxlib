package com.dannbrown.databoxlib.content.ship.extensions
...
fun Level.transformFromWorldToNearbyShipsAndWorld(aabb: AABB, cb: Consumer<AABB>) {
  ...
  getShipsIntersecting(aabb).forEach { ship ->
    cb.accept(tmpAABB.set(aabb)
      .transform(ship.worldToShip)
      .toMinecraft())
  }
}
...
fun Level?.getShipObjectManagingPos(pos: Vector3dc) =
  getShipObjectManagingPos(pos.x()
    .toInt() shr 4,
    pos.z()
      .toInt() shr 4)
...
fun ClientLevel?.transformRenderAABBToWorld(pos: Position, aabb: AABB): AABB {
  ...
  if (ship != null) {
    return aabb.toJOML()
      .transform(ship.renderTransform.shipToWorld)
      .toMinecraft()
  }
  return aabb
}

fun Entity.getShipManaging(): Ship? = this.level()
  .getShipManagingPos(this.position())
...
fun Level.getShipByShipId(shipId: ShipId): Ship? = allShips.getById(shipId)
fun getShipMountedToData(passenger: Entity, partialTicks: Float? = null): ShipMountedToData? {
  ...
  val shipObjectEntityMountedTo = passenger.level()
    .getShipObjectManagingPos(vehicle.position()
      .toJOML()) ?: return null
  val mountedPosInShip: Vector3dc = vehicle.getPosition(partialTicks ?: 0.0f)
    .add(0.0, vehicle.passengersRidingOffset + passenger.myRidingOffset, 0.0)
    .toJOML()
