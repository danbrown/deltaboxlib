...
class BuddingCottonBlock(properties: Properties?) : BuddingBushBlock(properties), BonemealableBlock {
  override fun getPlant(world: BlockGetter, pos: BlockPos): BlockState {
    return ProjectBlocks.BUDDING_COTTON_CROP.get().defaultBlockState()
  }
  ...
  override fun updateShape(
    ...
    if (state.getValue<Int>(AGE) == 4) {
      level.setBlock(currentPos, ProjectBlocks.COTTON_CROP.get().defaultBlockState(), 3)
    }
  ...
  override fun growPastMaxAge(state: BlockState?, level: ServerLevel?, pos: BlockPos?, random: RandomSource?) {
    level!!.setBlockAndUpdate(pos, ProjectBlocks.COTTON_CROP.get().defaultBlockState())
  }
  ...
  override fun performBonemeal(level: ServerLevel, random: RandomSource, pos: BlockPos, state: BlockState) {
    ...
    if (ageGrowth <= maxAge) {
      ...
      level.setBlockAndUpdate(
        pos,
        ProjectBlocks.COTTON_CROP.get().defaultBlockState().setValue(CottonCropBlock.CROP_AGE, remainingGrowth)
      )
    }
  }
}
