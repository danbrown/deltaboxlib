...
data class TrailParticleOption(val _target: Vec3, val _color: Int, val _duration: Int) : ParticleOptions {
  ...
  companion object {
    val DESERIALIZER: ParticleOptions.Deserializer<TrailParticleOption> =
      object : ParticleOptions.Deserializer<TrailParticleOption> {
        override fun fromCommand(
          particleType: ParticleType<TrailParticleOption>,
    ...
    val CODEC: Codec<TrailParticleOption> =
      RecordCodecBuilder.create { optionInstance: RecordCodecBuilder.Instance<TrailParticleOption> ->
        optionInstance.group(
