...
class LeavesParticle(pLevel: ClientLevel, pX: Double, pY: Double, pZ: Double, pSpriteSet: SpriteSet) :
  TextureSheetParticle(pLevel, pX, pY, pZ) {
  ...
  override fun tick() {
    ...
    if (!this.removed) {
      ...
      val f1 = min((f / 300.0f).toDouble(), 1.0)
        .toFloat()
      val d0: Double =
        cos(Math.toRadians((this.particleRandom * 60.0f).toDouble())) * 2.0 * Math.pow(f1.toDouble(), 1.25)
      val d1: Double =
        sin(Math.toRadians((this.particleRandom * 60.0f).toDouble())) * 2.0 * Math.pow(f1.toDouble(), 1.25)
