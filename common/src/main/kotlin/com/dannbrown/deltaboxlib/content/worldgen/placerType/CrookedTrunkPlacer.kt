...
class CrookedTrunkPlacer(pBaseHeight: Int, pHeightRandA: Int, pHeightRandB: Int) :
  ...
  override fun placeTrunk(
    ...
    placeLog(
      ...
      pBlockSetter,
      pRandom,
      mutableBlockPos.relative(direction.opposite),
      pConfig
    ) { state: BlockState -> state.setValue(RotatedPillarBlock.AXIS, direction.axis) }
    placeLog(
      pLevel,
      pBlockSetter,
      pRandom,
      mutableBlockPos.relative(if (pRandom.nextInt(2) == 0) direction.clockWise else direction.counterClockWise),
      pConfig
    )
  ...
  companion object {
    val CODEC: Codec<CrookedTrunkPlacer> =
      RecordCodecBuilder.create { placer: RecordCodecBuilder.Instance<CrookedTrunkPlacer> ->
        trunkPlacerParts(placer).apply(placer) { pBaseHeight: Int, pHeightRandA: Int, pHeightRandB: Int ->
          CrookedTrunkPlacer(
            pBaseHeight,
            pHeightRandA,
            pHeightRandB
